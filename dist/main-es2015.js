...
let AllKerryComponent = class AllKerryComponent {
    constructor(http, fs) {
        this.http = http;
        this.fs = fs;
    ...
    getOrderAllKerry() {
        ...
        this.http
            .getAllOrder()
            .then((result) => {
            this.dataTable = new _angular_material__WEBPACK_IMPORTED_MODULE_2__["MatTableDataSource"](result);
...
AllKerryComponent.ctorParameters = () => [
...
let FormJTComponent = class FormJTComponent {
    constructor() {
...
let FormKerryComponent = class FormKerryComponent {
    constructor(fb, fs, http, DatePipe, route, router, dialog) {
        this.fb = fb;
        this.fs = fs;
        this.http = http;
    ...
    addressFilter() {
        this.filteredOptions = this.kerryForm.controls["address2"].valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["startWith"])(""), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["map"])((value) => this.fs.filter(value)));
    }
    applyFilter(filterValue) {
        filterValue = filterValue.trim();
        filterValue = filterValue.toLowerCase();
    ...
    onDateChange() {
        this.kerryForm.controls["sendDate"].valueChanges.subscribe((value) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            if (!value)
                value = this.fs.getDeliveryDate;
            this.fs.setDeliveryDate = yield this.DatePipe.transform(value, "yyyy-MM-dd");
            if (!this.id)
                this.getIndex();
        }));
    }
    ...
    getIndex() {
        ...
        this.http
            ...
            .then((result) => {
            this.dataTable = new _angular_material__WEBPACK_IMPORTED_MODULE_8__["MatTableDataSource"](result.result1);
            this.dataTable.paginator = this.paginator;
            this.dataTable.sort = this.sort;
    ...
    onCreateOrder(model) {
        this.http
            .requestPost(model)
            .then(() => {
            this.fs.onNotifier("success", `(${this.kerryForm.value.mobile}) has been created successfully.`);
    ...
    onUpdateOrder(model) {
        ...
        this.http
            .requestPut(body)
            .then(() => {
            this.fs.onNotifier("info", `(${this.kerryForm.value.mobile}) has been updated successfully.`);
    ...
    onDeleteOrder(id, mobile) {
        this.fs.loading = true;
        this.http
            .requestDelete(id)
            .then(() => {
            this.fs.onNotifier("warning", `(${mobile}) has been deleted successfully`);
            this.getIndex();
        })
            .catch((error) => this.fs.onNotifier("error", error.message))
            .finally(() => (this.fs.loading = false));
    ...
    exportExcelFile() {
        ...
        this.http
            ...
            .catch((error) => {
            this.fs.onNotifier("error", error.error.message == undefined ? error.message : error.error.message);
        })
            .finally(() => {
            this.fs.loading = false;
...
FormKerryComponent.ctorParameters = () => [
...
let ReportDialog = class ReportDialog {
    constructor(fs, http, DatePipe, dialog) {
        this.fs = fs;
        this.http = http;
        this.DatePipe = DatePipe;
        this.dialog = dialog;
...
let ShopDialog = class ShopDialog {
    constructor(fs, http, DatePipe, dialog) {
        this.fs = fs;
        this.http = http;
        this.DatePipe = DatePipe;
